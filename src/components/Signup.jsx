import React, { useEffect, useState, useContext } from 'react';
import {
    TextField, makeStyles, Grid, Button, Paper, Card,
    CardContent, CardActions, Container, CardMedia,
    Typography
} from '@material-ui/core';
import { Link } from "react-router-dom";
import StepLabel from '@material-ui/core/StepLabel';
import BackupIcon from '@material-ui/icons/Backup';
import { AuthContext } from '../contexts/AuthContext';
import { storage, firestore, database } from "../firebase";
function Signup(props) {
    const [email, setEmail] = useState("");
    const [username, setUserName] = useState("");
    const [password, setPassword] = useState("");
    const [error, setError] = useState(false);
    const [loader, setLoader] = useState(false);
    const [file, setFile] = useState(null);
    let { signup } = useContext(AuthContext);
    let useStyle = makeStyles({
        centerDivs: {
            height: "100vh",
            width: "100vw",
            display: "flex",
            justifyContent: "center",
            alignItems: "center"
        },
        alignCenter: {
            width: "20rem",
            margin: "auto"
        },
        centerElements: {
            display: "flex",
            flexDirection: "column",
        },
        image: {
            height: "4rem",
            backgroundSize: "contain"
        }
    })

    const hiddenFileInput = React.useRef(null);
    function handleClick() {
        hiddenFileInput.current.click();
    };

    function handlFileSubmit(e) {
        let file = e?.target?.files[0];
        if (file != null) {
            console.log(e.target.files[0]);
            setFile(e.target.files[0]);
        }
    }
    async function handleSignup(e) {
        e.preventDefault();
        try {
            setLoader(true);
            //1
            let res = await signup(email, password);
            let uid = res.user.uid;
            const uploadTaskListener = storage.ref(`/users/${uid}/profileImage`).put(file);
            // fn1 -> progress
            // fn2 -> error 
            // fn3-> success
            uploadTaskListener.on('state_changed', fn1, fn2, fn3);
            function fn1(snapshot) {
                var progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
                console.log(progress);
            }
            function fn2(error) {
                setError(error);
                setLoader(false);
            }
            async function fn3() {
                // link get 
                let downloadurl = await
                    uploadTaskListener.snapshot.ref.getDownloadURL();
                database.users.doc(uid).set({
                    email: email,
                    userId: uid,
                    username,
                    createdAt: database.getUserTimeStamp(),
                    profileUrl: downloadurl
                })
                setLoader(false);
                props.history.push("/")
            }


        } catch (err) {
            setError("");
            setLoader(false);
        }
    }
    let classes = useStyle();
    return (<div className={classes.centerDivs}>
        <Container >
            <Grid container className={classes.alignCenter}  >
                <grid item sm={2} style={{ marginBottom: "0.5rem" }}>
                    <Card variant="outlined">
                        <CardMedia
                            image="data:image/png;base64,"
                            title="Instagram-logo"
                            className={classes.image}
                        />
                        <CardContent className={classes.centerElement} style={{ display: "grid" }}>
                            <Typography align="center" style={{ marginBottom: ".5rem", marginTop: "-1rem" }}>
                                Sign up to see photos and videos from your friends.
                            </Typography>
                            <TextField id="outlined-basic" type="email" label="Enter Email" display="block"
                                value={email} style={{ marginBottom: "0.5rem" }}
                                onChange={(e) => { setEmail(e.target.value) }} variant="outlined" size="small" />

                            <TextField id="outlined-basic" type="password" label="Password" display="block" variant="outlined" size="small"
                                value={password} style={{ marginBottom: "0.5rem" }} onChange={(e) => { setPassword(e.target.value) }} />

                            <TextField id="outlined-basic" type="username" label="Full Name" display="block"
                                value={username}
                                onChange={(e) => { setUserName(e.target.value) }} variant="outlined" size="small" />
                        </CardContent>
                        <CardActions style={{ display: "grid" }}>

                            <Button variant="outlined" style={{ marginBottom: ".5rem" }} color="secondary"
                                onClick={handleClick} startIcon={<BackupIcon></BackupIcon>}>UPLOAD PROFILE IMAGE</Button>

                            <input type="file"  ref={hiddenFileInput} className="image"

                                onChange={handlFileSubmit} style={{ display: "none" }} ></input>
                            <Button variant="contained" color="primary" onClick={handleSignup} disabled={loader} style={{ width: "90%", margin: "auto" }} >SIGN UP</Button>
                            <Typography align="center" style={{ marginTop: ".5rem", marginLeft: "0" }}>
                                By Signing up, you agree to our Terms, Data Policy and Cookies Policy.
                            </Typography>
                        </CardActions>
                    </Card>
                </grid>
                <Card variant="outlined" className={classes.alignCenter}>
                    <Typography style={{ textAlign: "center" }}>
                        Have an account?
                        <LinkButton
                            content="Log In"
                            route="/login"
                        ></LinkButton>
                    </Typography>
                </Card>
            </Grid>
        </Container>
        <div>
            {/*
                <div>
                    <label htmlFor="">Profile Image</label>
                    <input type="file" accept="image/*"

                        onChange={(e) => { handlFileSubmit(e) }}></input>
                </div>
                <button type="submit" disabled={loader}>Signup</button>
            </form> */}
        </div>
    </div>
    )
}

function LinkButton(prop) {
    return (
        <Button variant="text" style={{ color: "blue" }}>
            <Link to={prop.route}>
                {prop.content}
            </Link>
        </Button>

    )
}

export default Signup;